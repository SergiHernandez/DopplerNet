...
class kpts_decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.decoder_layers):
            if i == 0:
                #print("SHAPE at i==0:", x.shape) # Here the shape should be the same than after flattening: ([B, 4096])
                x = layer(x)
...
class GCN(nn.Module):
    def __init__(self, kpt_channels, gcn_channels, num_kpts=8, feature_size= 2048, is_gpu=True):

        super(GCN, self).__init__()

        self.kpts_decoder = kpts_decoder(features_size=feature_size,
